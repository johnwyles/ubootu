...
def test_ansible_syntax_check():
    ...
    result = subprocess.run(
        ["ansible-playbook", str(project_root / "site.yml"), "--syntax-check"],
        capture_output=True,
        text=True,
...
def test_ansible_check_mode():
    ...
    result = subprocess.run(
        [
            ...
            "/dev/null",  # Dummy password file for check mode
        ],
        capture_output=True,
        text=True,
        cwd=str(project_root),
...
def test_no_critical_failures_in_recent_run():
    ...
    for failure in failed_lines:
        if any(critical in failure for critical in ["is undefined", "apt_key", "syntax error", "duplicate loop"]):
...
def test_yaml_lint_passes():
    ...
    critical_files = ["site.yml", "roles/security/tasks/ssh.yml", "roles/desktop-environment/tasks/settings-gnome.yml"]

    for file_path in critical_files:
        full_path = project_root / file_path
        if full_path.exists():
            result = subprocess.run(["yamllint", str(full_path)], capture_output=True, text=True)
