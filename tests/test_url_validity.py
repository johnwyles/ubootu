            ...
            for pattern in url_patterns:
                ...
                for match in matches:
                    ...
                    if clean_url and clean_url not in urls:
                        urls.append({"url": clean_url, "file": str(yaml_file), "raw_match": match})
    ...
    for url_info in urls:
        ...
        try:
            ...
            if response.status_code >= 400:
                failed_urls.append({"url": url, "status": response.status_code, "file": url_info["file"]})

        except requests.RequestException as e:
            failed_urls.append({"url": url, "error": str(e), "file": url_info["file"]})
