        ...
        for i, line in enumerate(lines, 1):
            for pattern in apt_key_patterns:
                if pattern in line and not line.strip().startswith("#"):
                    found_patterns.append({"file": file_path, "line": i, "content": line.strip(), "pattern": pattern})
...
def test_signed_by_method_used():
    ...
    assert (
        len(signed_by_usage) > 0
    ), "No signed-by GPG method found. Repository setup may be missing proper GPG key handling."
