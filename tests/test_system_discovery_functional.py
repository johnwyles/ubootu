...
class TestSystemDiscoveryFunctional:
    ...
    def test_get_installed_snaps_success(self, mock_run):
        ...
        snap_data = [
            {"name": "code", "version": "1.85.1", "revision": "148", "channel": "stable", "publisher": "Microsoft"},
            {"name": "firefox", "version": "120.0.1", "revision": "3416", "channel": "stable", "publisher": "Mozilla"},
        ]

        mock_run.return_value = MagicMock(stdout=json.dumps(snap_data), returncode=0)
    ...
    def test_map_to_menu_items(self, mock_packages, mock_snaps, mock_flatpaks):
        ...
        mock_snaps.return_value = {"code": {"menu_id": "vscode", "version": "1.85.1"}}
    ...
    @patch(
        ...
        read_data="timestamp: '2024-01-01T00:00:00'\npackages:\n  firefox:\n    version: '120.0'\n",
    )
    @patch("pathlib.Path.exists")
    ...
    def test_is_safe_to_remove_has_dependencies(self, mock_run, mock_managed):
        ...
        mock_run.return_value = MagicMock(stdout="Reverse Depends:\n  firefox\n  thunderbird\n", returncode=0)
