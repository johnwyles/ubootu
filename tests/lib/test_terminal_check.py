...
class TestCheckTerminalCapabilities:
    ...
    @patch('sys.stdout.isatty', return_value=True)
    @patch.dict(os.environ, {'TERM': 'tmux-256color', 'TMUX': '/tmp/tmux-1000/default'}, clear=True)
    @patch('os.get_terminal_size')
...
class TestTestCursesBasic:
    ...
    def test_curses_with_colors(self, mock_init_pair, mock_start_color, 
                                mock_has_colors, mock_curs_set, mock_wrapper):
...
class TestCanRunTUI:
    ...
    @patch('lib.terminal_check.check_terminal_capabilities', 
    ...
    @patch('lib.terminal_check.check_terminal_capabilities', return_value=([], ["SSH detected"]))
    ...
    @patch('lib.terminal_check.check_terminal_capabilities', return_value=([], ["Warning 1", "Warning 2"]))
    @patch('lib.terminal_check.test_curses_basic', return_value=(True, None))
...
class TestPrintCompatibilityReport:
    ...
    @patch('lib.terminal_check.can_run_tui', 
    ...
    @patch('lib.terminal_check.can_run_tui', return_value=(True, [], ["SSH detected", "Small terminal"]))
    @patch.dict(os.environ, {'TERM': 'xterm'}, clear=True)
    @patch('os.get_terminal_size')
    @patch('locale.getlocale', return_value=(None, None))
