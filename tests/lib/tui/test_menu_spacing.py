...
class TestMenuSpacing(unittest.TestCase):
    ...
    def test_no_double_spaces_in_menu_items(self):
        ...
        for text in rendered_items:
            ...
            self.assertNotIn('  ', text, 
                f"Double space found in rendered text: '{text}'")
    ...
    def test_icon_fields_have_no_trailing_spaces(self):
        ...
        for item in items:
            if 'icon' in item:
                self.assertFalse(item['icon'].endswith(' '), 
                    f"Icon for {item['id']} has trailing space: '{item['icon']}'")
        ...
        for item in menu.items:
            if item.get('is_category'):
                ...
                self.assertNotIn('  ', rendered,
                    f"Double space found in: '{rendered}'")
        ...
        for cat in categories:
            if 'icon' in cat:
                ...
                self.assertFalse(icon.startswith(' '), 
                    f"Icon for {cat['id']} starts with space")
                self.assertFalse(icon.endswith(' '), 
                    f"Icon for {cat['id']} ends with space")
                
                # Icon should be 1-2 characters (emoji can be multi-byte)
                self.assertLessEqual(len(icon), 4,
                    f"Icon for {cat['id']} seems too long: '{icon}'")
