...
class TestSudoDialog(unittest.TestCase):
    ...
    def test_password_masking(self):
        ...
        password_chars = [ord('p'), ord('a'), ord('s'), ord('s'), 
                         ord('w'), ord('o'), ord('r'), ord('d'), ord('\n')]
        self.stdscr.getch.side_effect = password_chars
        
        password = dialog.get_password()
        
        # Check that asterisks were displayed
        displayed_calls = [call for call in self.stdscr.addstr.call_args_list 
                          if '*' in str(call)]
    ...
    def test_sudo_dialog_rendering(self):
        ...
        # Should draw a box
        box_calls = [call for call in self.stdscr.addch.call_args_list
                    if curses.ACS_ULCORNER in str(call)]
        self.assertGreater(len(box_calls), 0)
        
        # Should show the message
        msg_calls = [call for call in self.stdscr.addstr.call_args_list
                    if "Enter sudo password:" in str(call)]
    ...
    def test_dialog_centering(self):
        ...
        center_calls = [call for call in self.stdscr.addstr.call_args_list
                       if len(call[0]) >= 2 and 
                       abs(call[0][0] - expected_y) < 5 and
                       abs(call[0][1] - expected_x) < 10]
