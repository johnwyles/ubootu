...
class TestCompleteMenuStructure:
    ...
    def test_desktop_environments_complete(self):
        ...
        required_des = [
            ...
            'budgie', 'lxde', 'lxqt', 'enlightenment', 'deepin'
        ]
    ...
    def test_window_managers_complete(self):
        ...
        required_wms = [
            'hyprland', 'sway', 'i3', 'awesome', 'bspwm', 
            'dwm', 'openbox', 'qtile', 'river', 'wayfire',
            'herbstluftwm', 'spectrwm', 'xmonad', 'leftwm'
        ]
    ...
    def test_global_theme_system_exists(self):
        ...
        required_themes = [
            ...
            'one-dark', 'material', 'monokai-pro',
            'ayu-dark', 'ayu-light', 'everforest', 'rose-pine'
        ]
    ...
    def test_font_management_complete(self):
        ...
        font_items = ['font-interface', 'font-document', 'font-monospace', 
                      'font-title', 'font-antialiasing', 'font-hinting',
                      'font-size-scaling', 'nerd-fonts-pack']
        ...
        nerd_fonts_item = next((item for item in self.regular_items 
                               if item['id'] == 'nerd-fonts-pack'), None)
        if nerd_fonts_item and nerd_fonts_item.get('config_options'):
            required_fonts = ['jetbrains-mono', 'hack', 'fira-code', 
    ...
    def test_security_tools_complete(self):
        ...
        security_tools = [
            ...
            'hashcat', 'aircrack-ng', 'hydra', 'sqlmap', 'nikto',
            'dirb', 'gobuster', 'ffuf', 'zaproxy', 'beef'
        ]
        
        privacy_tools = [
            'tor-browser', 'veracrypt', 'keepassxc', 'bitwarden',
            'protonvpn', 'mullvad-vpn', 'signal', 'mat2', 'bleachbit'
        ]
    ...
    def test_development_tools_complete(self):
        ...
        ides = [
            'vscode', 'pycharm', 'intellij-idea', 'webstorm', 'goland', 
            'clion', 'rider', 'datagrip', 'android-studio', 'cursor', 
            'zed', 'sublime-text', 'vim', 'neovim', 'emacs'
        ]
        ...
        containers = [
            ...
            'minikube', 'k9s', 'lens', 'helm', 'kubectl'
        ]
    ...
    def test_gaming_category_exists(self):
        ...
        gaming_tools = [
            ...
            'gamemode', 'mangohud', 'goverlay', 'protontricks'
        ]
    ...
    def test_multimedia_production_tools(self):
        ...
        multimedia = [
            'obs-studio', 'kdenlive', 'shotcut', 'openshot', 'davinci-resolve',
            'audacity', 'ardour', 'lmms', 'hydrogen', 'bitwig-studio',
            'blender', 'freecad', 'openscad', 'krita', 'inkscape'
        ]
    ...
    def test_system_settings_complete(self):
        ...
        mouse_settings = [
            'mouse-speed', 'mouse-acceleration', 'scroll-speed', 
            'natural-scroll', 'touchpad-tap-click', 'touchpad-gestures'
        ]
        
        display_settings = [
            'display-scaling', 'display-arrangement', 'refresh-rate',
            'night-light', 'color-profile'
        ]
        
        power_settings = [
            'power-profile', 'suspend-on-lid-close', 'battery-percentage',
            'cpu-governor', 'tlp-config'
        ]
    ...
    def test_shell_configuration(self):
        ...
        prompts = [
            'prompt-starship', 'prompt-ohmyposh', 'prompt-pure',
    ...
    def test_cloud_tools_complete(self):
        ...
        cloud_tools = [
            'aws-cli', 'gcloud', 'azure-cli', 'terraform', 'pulumi',
            'ansible', 'vagrant', 'packer', 'vault', 'consul'
        ]
    ...
    def test_ansible_var_mappings(self):
        ...
        system_items = [item for item in self.regular_items 
                       if item.get('parent') in ['system', 'desktop', 'security']]
        
        items_without_var = [item['id'] for item in system_items 
                           if not item.get('ansible_var') and not item.get('is_configurable')]
        
        # Allow some items without ansible_var, but not too many
        assert len(items_without_var) < len(system_items) * 0.3, \
