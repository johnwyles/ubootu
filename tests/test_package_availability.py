...
PROBLEMATIC_PACKAGES = ["bandwhich", "speedtest", "dbeaver-ce", "paris-traceroute", "tailscale", "gimp-plugin-registry"]

# Required packages that should be available
REQUIRED_PACKAGES = ["flatpak", "python3-psutil", "python3-apt", "python3-gpg"]
    ...
    for package in PROBLEMATIC_PACKAGES:
        ...
        if result.returncode != 0:
            ...
            if alternative is None:
                ...
                snap_result = subprocess.run(["snap", "info", snap_package], capture_output=True, text=True)
                assert (
                    snap_result.returncode == 0
                ), f"Package '{package}' not available and snap alternative '{snap_package}' also not found"
            else:
                # Test if alternative package is available
                alt_result = subprocess.run(["apt-cache", "show", alternative], capture_output=True, text=True)
                assert (
                    alt_result.returncode == 0
                ), f"Package '{package}' not available and alternative '{alternative}' also not found"
...
def test_flatpak_installation():
    ...
    result = subprocess.run(["apt-cache", "show", "flatpak"], capture_output=True, text=True)
...
def test_ansible_package_syntax():
    ...
    # Test syntax
    result = subprocess.run(["ansible-playbook", "--syntax-check", playbook_path], capture_output=True, text=True)
