...
class TestFullAnsibleRun:
    ...
    def test_config(self):
        ...
        return {
            "selected_items": ["dev-vcs", "git", "system-monitoring", "htop", "btop"],
            "configurable_items": {"swappiness": {"id": "swappiness", "value": 10}},
            "ansible_variables": {
...
"""
    ...
    def test_ansible_syntax_check(self, ansible_inventory, extra_vars_file):
        ...
        cmd = ["ansible-playbook", "site.yml", "--syntax-check", "-i", ansible_inventory, "-e", f"@{extra_vars_file}"]

        result = subprocess.run(cmd, capture_output=True, text=True, cwd="/home/jwyles/code/ubootu")
...
"""
        ...
        try:
            result = subprocess.run(
                ["ansible-playbook", playbook_path, "-i", ansible_inventory], capture_output=True, text=True
            )
    ...
    def test_no_failed_tasks(self, ansible_inventory, extra_vars_file):
        ...
        cmd = [
            ...
            "--skip-tags",
            "never_check",  # Skip tasks that can't run in check mode
        ]
