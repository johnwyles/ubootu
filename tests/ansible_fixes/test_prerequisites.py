...
class TestSystemPrerequisites:
    ...
    def test_python3_psutil_installed_early(self):
        ...
        prerequisites = ["python3-psutil", "python3-apt", "python3-gpg", "curl", "gpg", "software-properties-common"]

        # Test that these are installed in common/tasks/packages.yml
        common_packages_order = ["apt_update", "essential_packages", "other_tasks"]  # Should include above packages
    ...
    def test_check_before_modify_pattern(self):
        ...
        check_patterns = [
            ...
            {"check": "command -v flatpak", "condition": "flatpak_installed.rc == 0", "task": "flatpak_operations"},
            {"check": "getent group docker", "condition": "docker_group.rc == 0", "task": "add_user_to_docker_group"},
        ]
...
class TestPackageDependencies:
    ...
    def test_build_dependencies_for_source_builds(self):
        ...
        build_deps = {
            "common": ["build-essential", "git", "curl", "wget"],
            "rust": ["cargo", "rustc"],
            "go": ["golang-go"],
        }
    ...
    def test_alternative_installation_methods(self):
        ...
        alternative_installs = {
            "dog": {"method": "snap", "package": "dog"},
            "insomnia": {"method": "deb_download", "url": "https://updates.insomnia.rest/downloads/ubuntu/latest"},
            "dbeaver-ce": {"method": "apt_repository", "repo": "dbeaver"},
        }
...
class TestOrderingDependencies:
    ...
    def test_repository_before_package_install(self):
        ...
        task_order = ["add_docker_repository", "apt_update", "install_docker_ce"]
    ...
    def test_gpg_key_before_repository(self):
        ...
        task_order = ["install_gpg_package", "download_repo_key", "add_repository", "apt_update", "install_package"]
    ...
    def test_user_exists_before_user_operations(self):
        ...
        user_tasks = {
            "create_user": {"name": "Ensure user exists", "user": {"name": "{{ primary_user }}", "state": "present"}},
            "add_to_group": {
                "name": "Add user to groups",
                "user": {"name": "{{ primary_user }}", "groups": "docker,sudo", "append": True},
...
class TestErrorHandling:
    ...
    def test_missing_package_fallback(self):
        ...
        fallback_strategies = {
            "package_not_found": {"primary": "apt", "fallback": ["snap", "flatpak", "manual_download"]},
            "repository_not_available": {"primary": "current_release", "fallback": ["previous_lts", "manual_download"]},
        }
