...
def test_git_editor_variable():
    ...
    test_playbook = [
        {
            "hosts": "localhost",
            "vars": {"code_editors": ["vscode", "vim"]},
            "tasks": [
                {
                    "name": "Test git editor setting",
                    "debug": {"msg": "{{ 'code --wait' if 'vscode' in code_editors else 'vim' }}"},
                }
            ],
        }
    ]
...
def test_role_defaults_exist():
    ...
    critical_roles = ["development-tools", "dotfiles", "applications"]
    ...
    for role_name in critical_roles:
        ...
        if defaults_file.exists():
            ...
            if role_name == "development-tools":
                required_vars = ["devtools_selected_file_managers", "devtools_selected_modern_replacements"]
                for var in required_vars:
                    assert var in content, f"Variable '{var}' not found in {role_name} defaults"

            elif role_name == "dotfiles":
                assert (
                    "code_editors" in content or "prompt_decorator" in content
                ), f"Git-related variables not found in {role_name} defaults"
...
def test_ansible_undefined_variable_check():
    ...
    test_vars = {"primary_user": "test", "selected_items": ["git"], "desktop_environment": "gnome"}
    ...
    result = subprocess.run(
        ["ansible-playbook", str(project_root / "site.yml"), "--syntax-check", "-e", f"@{vars_file}"],
        capture_output=True,
        text=True,
