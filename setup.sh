...
sections = menu.show_section_selection()

# Save selected sections to temp file
...
if profiles['saved']:
    print("\nSaved Profiles:")
    for p in profiles['saved']:
        print(f"  {idx}. {p}")
        all_profiles.append(('saved', p))
        idx += 1

# Show templates
if profiles['templates']:
    print("\nTemplate Profiles:")
    for p in profiles['templates']:
        print(f"  {idx}. {p}")
        all_profiles.append(('templates', p))
        idx += 1

choice = input("\nSelect profile number (or 'c' to cancel): ")

if choice.lower() != 'c' and choice.isdigit():
    choice_idx = int(choice) - 1
    if 0 <= choice_idx < len(all_profiles):
        profile_type, profile_name = all_profiles[choice_idx]
...
name = input("Enter backup name (or press Enter for timestamp): ").strip()
if not name:
...
try:
    ...
    sync = input("\nPush to remote repository? (y/N): ").lower()
    if sync == 'y':
        if profile_mgr.push_to_remote():
            print("Backup pushed to remote repository")
        else:
            print("Failed to push to remote (is remote configured?)")
...
profile_mgr = ProfileManager()
history = profile_mgr.get_history(20)
...
print("\nOptions:")
print("1. Restore from a commit")
print("2. View diff between commits")
print("3. Back to main menu")

choice = input("\nEnter choice: ")

if choice == '1':
    commit = input("Enter commit hash: ")
    if profile_mgr.restore_from_commit(commit):
        print(f"Restored configuration from {commit}")
    else:
        print("Failed to restore configuration")
        
elif choice == '2':
    commit1 = input("Enter first commit hash (or 'current'): ")
    commit2 = input("Enter second commit hash: ")
    diff = profile_mgr.diff_profiles(
        commit1 if commit1 != 'current' else None,
        commit2
    )
    print("\nDifferences:")
    print(diff)
...
quick_actions() {
    ...
    echo "5. Reset application defaults"
    ...
    case "$choice" in
        ...
        4)
            ...
            ;;
        5)
            print_info "Resetting application defaults..."
            ansible-playbook site.yml --tags "defaults" --ask-become-pass
            ;;
    esac
}
...
Command Line Usage:
  ...
  ./setup.sh --restore FILE    Restore from configuration file
  ./setup.sh --backup          Backup current configuration
  ./setup.sh --help            Show this help

Configuration Files:
  config.yml                   Current configuration
  ~/.config/unbootu/          Profile storage location
...
For more information, visit:
  https://github.com/yourusername/unbootu
...
main() {
    ...
    # If user chose to exit, exit gracefully
    if [[ "$welcome_choice" == "8" ]]; then
        print_info "Thanks for using Ubootu! 🚀"
        exit 0
    fi
    
    # Explain why we need sudo and install prerequisites
    explain_sudo_requirement
    install_prerequisites
    
    # Handle the welcome menu choice directly
    case "$welcome_choice" in
        ...
        7)  # Help
            ...
            exit 0
            ;;
        *)  # Default to main menu loop
            ;;
    esac
    
    # Main menu loop (fallback - shouldn't normally reach here)
    while true; do
