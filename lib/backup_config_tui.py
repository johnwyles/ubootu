    ...
    try:
        ...
        if sync_remote:
            ...
            if profile_mgr.push_to_remote():
                ...
                msg_dialog.show("Success", f"Backup '{backup_name}' created and pushed to remote", "info")
            else:
                msg_dialog = MessageDialog(stdscr)
                msg_dialog.show("Warning", f"Backup '{backup_name}' created locally but failed to push to remote", "warning")
        else:
            msg_dialog = MessageDialog(stdscr)
