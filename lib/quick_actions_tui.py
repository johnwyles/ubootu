...
class QuickActions:
    ...
    def update_all_packages(self):
        ...
        if not failed_results:
            ...
            if packages_upgraded > 0:
                msg.show("Success", f"Updated {packages_upgraded} packages successfully", "info")
            else:
    ...
    def clean_system(self):
        ...
        if not failed_results:
            msg = MessageDialog(self.stdscr)
            msg.show("Success", f"System cleaned successfully. Freed {space_freed} of disk space.", "info")
        else:
    ...
    def check_system_health(self):
        ...
        result = self.run_command("apt list --upgradable 2>/dev/null | grep -i security | wc -l", "Check security updates")
        if result.success:
            try:
    ...
    def security_audit(self):
        ...
        result = self.run_command("sudo journalctl -u ssh --since '1 week ago' | grep -i 'failed\\|invalid' | wc -l", "Check failed logins")
        if result.success:
            try:
        ...
        result = self.run_command("grep -E '^PASS_MAX_DAYS|^PASS_MIN_DAYS|^PASS_MIN_LEN' /etc/login.defs", "Check password policy")
        if result.success:
        ...
        msg = MessageDialog(self.stdscr)
        msg.show("Security Audit Results", "\n".join(security_info), "info" if security_score >= 80 else "warning")
