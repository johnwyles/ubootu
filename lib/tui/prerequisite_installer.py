...
Continue with installation?"""
        ...
        return self.confirm_dialog.show("Administrator Access Required", explanation)
    ...
    def _run_task(self, index: int, task_name: str, task_func: callable) -> bool:
        ...
        try:
            ...
            self._update_task_status(index, "failed")
            self.message_dialog.show("Task Failed", f"Error in {task_name}:\n{str(e)}", "error")
            return False
    ...
    def _check_apt_health(self) -> bool:
        ...
        if apt_fixer_path.exists():
            exit_code = self.progress_dialog.run_command(
                [sys.executable, str(apt_fixer_path)], "Checking APT Health", show_output=True
            )
            return exit_code == 0
        else:
    ...
    def _run_apt_check(self) -> bool:
        ...
        exit_code = self.progress_dialog.run_command(["apt", "--version"], "Checking APT", show_output=False)
        return exit_code == 0

    def _update_apt_cache(self) -> bool:
        """Update APT package cache"""
        exit_code = self.progress_dialog.run_command(
            ["sudo", "apt", "update"], "Updating Package Cache", show_output=True, sudo_dialog=self.sudo_dialog
        )
        return exit_code == 0
    ...
    def _install_python(self) -> bool:
        ...
        check_result = subprocess.run(["which", "python3"], capture_output=True)
    ...
    def _install_ansible(self) -> bool:
        """Install Ansible"""
        # Check if already installed
        check_result = subprocess.run(["which", "ansible"], capture_output=True)
        ...
        if codename in ["focal", "jammy", "noble"]:
            ...
            self.progress_dialog.show_message(
                "Installing Ansible", "Adding Ansible PPA repository...", wait_for_key=False
            )
    ...
    def _install_python_packages(self) -> bool:
        ...
        exit_code = self.progress_dialog.run_command(
            [pip_cmd, "install", "--user"] + packages, "Installing Python Packages", show_output=True
        )
        return exit_code == 0
    ...
    def _install_ansible_collections(self) -> bool:
        ...
        exit_code = self.progress_dialog.run_command(
            ["ansible-galaxy", "install", "-r", "requirements.yml"], "Installing Ansible Collections", show_output=True
        )
        return exit_code == 0
