...
class ProfileSelector:
    ...
    def render_profiles(self) -> None:
        ...
        if not self.profiles:
            ...
            draw_centered_text(self.stdscr, self.height // 2 + 2, 
                             "Create profiles by saving configurations")
            return
    ...
    def delete_profile(self) -> bool:
        ...
        if dialog.show(f"Delete profile '{profile['name']}'?", 
    ...
    def apply_profile(self) -> Optional[str]:
        ...
        # Confirm application
        dialog = ConfirmDialog(self.stdscr)
        if dialog.show(f"Apply profile '{profile['name']}'?", 
                      f"This will use the configuration from {profile['created_at']}"):
