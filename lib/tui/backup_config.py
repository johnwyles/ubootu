...
class BackupConfig:
    ...
    def render_instructions(self) -> None:
        ...
        if not self.config_file.exists():
            draw_centered_text(self.stdscr, self.height // 2, "No configuration found to backup!")
            draw_centered_text(
                self.stdscr, self.height // 2 + 2, "Create a configuration first using Fresh Install or Modify Setup"
            )
            return
        ...
        try:
            ...
            draw_centered_text(self.stdscr, self.height // 2, f"Error reading configuration: {str(e)}")
    ...
    def get_system_info(self) -> dict:
        ...
        info = {"hostname": os.uname().nodename, "version": "Unknown", "kernel": os.uname().release}
    ...
    def create_backup(self) -> bool:
        ...
        profile_name = input_dialog.show(
            "Profile Name",
        ...
        # Get description
        description = input_dialog.show(
            "Profile Description", "Enter a description (optional):", default="My Ubuntu configuration"
        )
        ...
        if profile_path.exists():
            ...
            if not confirm_dialog.show("Profile Exists", f"Profile '{safe_name}' already exists. Overwrite?"):
                return False
        ...
        try:
            ...
            msg_dialog.show(
                "Success", f"Profile '{profile_name}' created successfully!\n\n" f"Saved to: {profile_path}"
            )
            return True
