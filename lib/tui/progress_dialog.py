...
class ProgressDialog:
    ...
    def run_command(self, command: List[str], title: str = "Processing...", 
        ...
        while self.is_running or not self.output_queue.empty():
            ...
            if (new_lines_added or 
                self.last_task != self.current_task or
                self.last_completed_tasks != self.completed_tasks or
                self.last_skipped_tasks != self.skipped_tasks or
                self.needs_redraw):
        ...
        try:
            while True:
                ...
                if total_time > max_total_time:
                    self.output_queue.put(f"ERROR: Process exceeded maximum time limit ({max_total_time/60:.0f} minutes)")
