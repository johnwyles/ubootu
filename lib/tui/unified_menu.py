...
from .dialogs import ConfirmDialog, HelpDialog, MessageDialog, SelectDialog, SliderDialog, SpinnerDialog
...
class UnifiedMenu:
    ...
    def save_configuration(self, silent: bool = False) -> bool:
        ...
        """
        config = {
            "metadata": {"version": "1.0", "created_by": "ubootu_unified_tui"},
            "selected_items": [],
            "configurable_items": {},
        }
        ...
        for item_id, value in self.configurable_values.items():
            config["configurable_items"][item_id] = {"id": item_id, "value": value}
    ...
    def render_header(self) -> None:
        ...
        if self.discovery and self.system_state:
            needs_install = sum(
                1 for item_id in self.system_state if self.get_item_sync_status(item_id) == "needs_install"
            )
            orphaned = sum(1 for item_id in self.system_state if self.get_item_sync_status(item_id) == "orphaned")
    ...
    def render_menu_item(self, y: int, item: Dict, selected: bool) -> None:
        ...
        if item.get("is_category"):
            ...
            indicator = self.get_selection_indicator(item["id"], self.selections.get(item["id"], set()))
            icon = item.get("icon", "")
            ...
            if item.get("is_configurable"):
                # Show current value
                current_value = self.configurable_values.get(item["id"], item.get("default_value", ""))
                unit = item.get("unit", "")
                if unit:
    ...
    def toggle_selection(self) -> None:
        ...
        if item.get("is_category"):
            ...
            if descendants:
                ...
                any_selected = any(self.is_item_selected(desc_id) for desc_id in descendants)
    ...
    def show_config_dialog(self, item: Dict) -> None:
        ...
        if config_type == "slider":
            ...
            if values:
                new_value = dialog.show(item["label"], current_value, values, item.get("unit", ""))
        elif config_type == "select":
            dialog = SelectDialog(self.stdscr)
            options = item.get("options", [])
            if options:
                selected_index = dialog.show(
                    item["label"], None, options, str(current_value)  # No message for this dialog
                )
            ...
            new_value = dialog.show(item["label"], item.get("description", "Enter value:"), str(current_value))
    ...
    def show_diff_dialog(self) -> None:
        ...
        # Show in a dialog
        MessageDialog(self.stdscr).show("System Diff", "".join(lines), "info")
    ...
    def apply_configuration(self) -> bool:
        ...
        if not Path("config.yml").exists():
            MessageDialog(self.stdscr).show(
                "No Configuration", "No configuration file found. Please save your selections first.", "error"
            )
            return False
        ...
        sudo_password = sudo_dialog.get_password("Enter your sudo password to install selected software:")
...
"""
        ...
        if result == 0:
            ...
            MessageDialog(self.stdscr).show("Configuration Applied", summary, "success")
            return True
        else:
            ...
            MessageDialog(self.stdscr).show("Configuration Failed", failure_summary + debug_info, "error")
            return False
    ...
    def _get_config_data(self) -> Dict[str, Any]:
        ...
        configurable_items = {}
        for item_id, value in self.configurable_values.items():
            configurable_items[item_id] = {"id": item_id, "value": value}

        return {"selected_items": sorted(selected_items), "configurable_items": configurable_items}
