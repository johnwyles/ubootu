...
class EnhancedMenuUI:
    ...
    def _build_menu_structure(self):
        ...
        self.menu_items["action-install"] = MenuItem(
            "action-install", "🚀 Start Installation", "Apply settings and install selected software", parent="actions"
        )

        self.menu_items["action-save"] = MenuItem(
            "action-save", "💾 Save Configuration", "Save current selections without installing", parent="actions"
        )

        self.menu_items["action-reset"] = MenuItem(
            "action-reset", "🔄 Reset Configuration", "Clear all selections and return to defaults", parent="actions"
        )

        self.menu_items["action-exit"] = MenuItem(
            "action-exit", "❌ Exit without Saving", "Exit the configuration tool without saving", parent="actions"
        )
    ...
    def _build_ides_menu(self):
        ...
        for ide_id, label, desc, default in ides:
            self.menu_items[ide_id] = MenuItem(ide_id, label, desc, parent="dev-ides", default=default)
    ...
    def _build_languages_menu(self):
        ...
        for lang_id, label, desc, default in languages:
            self.menu_items[lang_id] = MenuItem(lang_id, label, desc, parent="dev-languages", default=default)
    ...
    def _build_dev_tools_menu(self):
        ...
        for tool_id, label, desc, default in tools:
            self.menu_items[tool_id] = MenuItem(tool_id, label, desc, parent="dev-tools", default=default)
    ...
    def _build_containers_menu(self):
        ...
        for container_id, label, desc, default in containers:
            self.menu_items[container_id] = MenuItem(
                container_id, label, desc, parent="dev-containers", default=default
            )
    ...
    def _build_modern_cli_menu(self):
        ...
        for cli_id, label, desc, default in cli_tools:
            self.menu_items[cli_id] = MenuItem(cli_id, label, desc, parent="dev-cli-modern", default=default)
    ...
    def _build_desktop_menu(self):
        ...
        for desktop_id, label, desc, default in desktops:
            self.menu_items[desktop_id] = MenuItem(
                desktop_id, label, desc, parent="desktop-environments", default=default
            )
        ...
        for theme_id, label, desc, default in themes:
            self.menu_items[theme_id] = MenuItem(theme_id, label, desc, parent="themes", default=default)
    ...
    def _build_applications_menu(self):
        ...
        for browser_id, label, desc, default in browsers:
            self.menu_items[browser_id] = MenuItem(browser_id, label, desc, parent="app-browsers", default=default)
    ...
    def _build_security_menu(self):
        ...
        for sec_id, label, desc, default in security_items:
            self.menu_items[sec_id] = MenuItem(sec_id, label, desc, parent="security-firewall", default=default)
    ...
    def _build_system_menu(self):
        ...
        for sys_id, label, desc, default in system_items:
            self.menu_items[sys_id] = MenuItem(sys_id, label, desc, parent="system-performance", default=default)
        ...
        try:
            ...
            while True:
                ...
                for i, item in enumerate(menu_items):
                    ...
                    table.add_row(
                        f"[{style}]{indicator}[/]", f"[{style}]{item.label}[/]", f"[{style} dim]{item.description}[/]"
                    )
